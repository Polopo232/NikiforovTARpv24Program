...
# 1. Andmebaasi loomine normaliseeritud tabelitega
def create_table():
    try:
...
def insert_initial_data():
    try:
        ...
        # Пример вставки одного фильма (лучше циклом, через SELECT id FROM ...)
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            ("The From In With.", director_id, 1994, genre_id, 142, 9.3, language_id, country_id, "Описание...")
        )
...
def load_data_to_treeview():
    try:
        ...
        # Lisa algandmed, kui puuduvad
        for table in ['languages', 'countries', 'genres', 'directors']:
            cursor.execute(f"SELECT COUNT(*) FROM {table}")
            if cursor.fetchone()[0] == 0:
                if table == 'languages':
                    cursor.executemany("INSERT INTO languages (name) VALUES (?)", 
                                     [('English',), ('Estonian',), ('French',), ('German',)])
                elif table == 'countries':
                    cursor.executemany("INSERT INTO countries (name) VALUES (?)", 
                                     [('USA',), ('UK',), ('France',), ('Germany',)])
                elif table == 'genres':
                    cursor.executemany("INSERT INTO genres (name) VALUES (?)", 
                                     [('Drama',), ('Sci-Fi',), ('Crime',), ('Comedy',)])
                elif table == 'directors':
                    cursor.executemany("INSERT INTO directors (name) VALUES (?)", 
                                     [('Francis Ford Coppola',), ('Christopher Nolan',), 
                                      ('Quentin Tarantino',), ('Steven Spielberg',)])
        conn.commit()

    except sqlite3.Error as error:
        print("Viga andmete laadimisel:", error)
    finally:
        if conn:
            conn.close()
# 2. Lisaaken uute andmete lisamiseks
def lisa_aken():
    aken = tk.Toplevel()
    ...
    # Combobox-id seotud andmete valimiseks
    ...
    def salvesta():
        # Siia tuleks lisada andmete salvestamise loogika
...
# 3. Funktsioonid uute andmete lisamiseks
...
# Andmebaasi loomine ja täitmine
create_table()
insert_initial_data()
